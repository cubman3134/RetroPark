...
TEST_CASE("backend: interface is abstract and includes cleanly") {
    ...
    struct Stub : rp::IRenderBackend {
        ...
        rp_result composite_and_present(uint32_t, uint64_t, bool, uint8_t*, std::string&) override { return RP_OK; }
        rp_result composite_driven(const void*, uint32_t, uint32_t, uint32_t, bool, uint8_t*, std::string&) override { return RP_OK; }
    } s;
    std::string e;
